...
async def create_researcher(researcher_data: ResearcherCreate, database: Session = Depends(get_db)) -> Researcher:
    ...
    db_researcher = Researcher(
        hIndex=researcher_data.hIndex,        totalPapers=researcher_data.totalPapers,        careerAge=researcher_data.careerAge,        citationsPerPaper=researcher_data.citationsPerPaper,        totalCitations=researcher_data.totalCitations,        researchfield_id=researcher_data.researchfield        )
...
async def bulk_create_researcher(items: list[ResearcherCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_researcher = Researcher(
                hIndex=item_data.hIndex,                totalPapers=item_data.totalPapers,                careerAge=item_data.careerAge,                citationsPerPaper=item_data.citationsPerPaper,                totalCitations=item_data.totalCitations,                researchfield_id=item_data.researchfield            )
            database.add(db_researcher)
